...
void pqMeter(int selectionBarHeight) {
  ...
  for (int ch = 0; ch < numChannels; ch++) {
    float peakValue = 0;
    float rmsSum = 0;
    float[][] scaledData = applyScale(plotChannelData); // VERIFICAR!!!!
  ...
  text(nf(iMagA, 2, 2) + "A", Pos4, 7*AuxB+AuxD/2);
  text(nf(iMagA, 2, 2) + "A", Pos6, 7*AuxB+AuxD/2);
