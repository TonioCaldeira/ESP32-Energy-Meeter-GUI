// Função para desenhar os gráficos no domínio do tempo
...
// Função para desenhar os gráficos no domínio da frequência
...
// Função para desenhar o diagrama fasorial
void drawPhasorDiagram(int startX, int startY, int sectionWidth, int sectionHeight, int harmonicIndex) {
  ...
  float ref = 0;
  float absoluteThreshold = 5; // Valor absoluto para ignorar magnitudes < 5%
...
// Função para atualizar o número de amostras a serem exibidas
void updateDisplaySamples() {
  displaySamples = (int)(cyclesToDisplay * UDPRateAverage * samplePerChannel / 60);
  sampleRate = (int)(UDPRateAverage * samplePerChannel);
}
...
void drawMultiplicationPlot(int startX, int startY, int plotWidth, int plotHeight, int channelA, int channelB, int R, int G, int B) {
  ...
  // Define a cor para o plot de multiplicação
  stroke(R, G, B);
  strokeWeight(3);
  noFill();
  beginShape();
...
void drawElectricalParameters(int startX, int startY, int channelA, int channelB) {
  ...
  float sumCurrentSq// Função para desenhar os gráficos no domínio do tempo
void drawTimeDomain(int width, int selectionBarHeight) {
  int columns = 2; // Número de colunas
  int rows = 3;    // Número de linhas (número de canais por coluna)
